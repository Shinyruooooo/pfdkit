...
def _link_input(iname: str, target: Path) -> None:
    r"""Symlink `iname` to `target`, replacing a stale symlink if present.

    Re-execution in the same working directory (e.g. dflow retry after a
    TransientError) leaves the links from the previous attempt behind;
    without this, symlink_to raises FileExistsError and the retry fails
    before the FP command ever runs.
    """
    if os.path.islink(iname):
        os.unlink(iname)
    elif os.path.exists(iname):
        raise FatalError(
            f"{iname} already exists and is not a symlink; refusing to overwrite"
        )
    Path(iname).symlink_to(target)
