...
def dpdata2ase(
    ...
    for ii in range(len(sys)):
        ...
        if "virials" in sys[ii].data:
            # virial is a per-configuration 3x3 tensor, store it in atoms.info
            # (not atoms.arrays, which is for per-atom data)
            atoms.info["virial"] = np.asarray(sys[ii].data["virials"][0]).reshape(3, 3)
...
def ase2dpdata(
    ...
    if labeled:
        ...
        if "virial" in atoms.info:
            # virial is per-configuration and lives in atoms.info; dpdata
            # expects the plural key "virials" with shape (nframes, 3, 3)
            virials = np.asarray(atoms.info["virial"]).reshape(3, 3)
